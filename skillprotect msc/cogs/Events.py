        ...
        if data["automoderation"] is True:
            if ghostping is True:
                if len(message.mentions) == 0:
                    ...
                    ghostping = discord.Embed(title=f'Призрак пингуется!', color=discord.Colour.blue())
                    ghostping.add_field(name='Имя', value=f"{message.author.mention}")
                    ghostping.add_field(name='Сообщение', value=f'{message.content}', inline=False)
                    try:
