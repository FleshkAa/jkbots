                    ...
                    try:
                        if antiSpam is True:
                            ...
                            if len(list(filter(lambda m: check(m), self.client.cached_messages))) >= 4 and len(list(filter(lambda m: check(m), self.client.cached_messages))) < 8:
                                ...
                                if data["punishment"] == "kick":
                                    await message.author.ban(reason=f"Автоматическая модерация безопасности сервера | Спам", delete_message_days=7)
                        ...
                        if antiLink is True:
                            ...
                            if "https://" in message.content:
                                ...
                                if data["punishment"] == "kick":
                                    reason = "Send a link"
                                    await message.author.kick(reason=f"Автоматическая модерация безопасности сервера | {reason}")

                                if data["punishment"] == "ban":
                                    await message.author.ban(reason=f"Автоматическая модерация безопасности сервера | {reason}", delete_message_days=0)
                        ...
                        if antiWord is True:
                            ...
                            with open('badwords.txt') as BadWords:
                                if msg in BadWords.read():
                                    await message.delete()
                                    await message.channel.send("Не используйте это здесь!")

                        if message.mention_everyone:
                            await ctx.send("Вы не можете упомянуть всех")
